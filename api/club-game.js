...
async function loadRoom(client, roomId, { forUpdate = false } = {}) {
  const { rows: [room] } = await client.query(
    forUpdate
      ? `SELECT * FROM club_rooms WHERE id = $1 FOR UPDATE`
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "PATCH") {
      ...
      // Lock the room row for the whole PATCH so concurrent submit_answer
      // cannot overwrite each other and "lose" a submission (name flips red,
      // judging never starts until that player submits again).
      await client.query("BEGIN");
      let committed = false;
      try {
        const data = await loadRoom(client, room_id, { forUpdate: true });
        if (!data) {
        ...
        try {
          ...
          } else if (action === "clear_answer") {
            gameState = applyAction(gameState, "clear_answer", user);
