...
describe("club-engine", () => {
  ...
  it("clear_answer removes submission and stays in answer phase", () => {
    let s = createInitialState(["alice", "bob", "carol"], { host: "alice" });
    s = applyAction(s, "submit_answer", "alice", { imageDataUrl: "data:img,a1" });
    assert.equal(publicState(s, "bob").players.find((p) => p.username === "alice").hasSubmitted, true);
    s = applyAction(s, "clear_answer", "alice");
    assert.equal(s.submissions.alice, undefined);
    assert.equal(s.phase, "answer");
    assert.equal(publicState(s, "bob").players.find((p) => p.username === "alice").hasSubmitted, false);
    s = applyAction(s, "submit_answer", "bob", { imageDataUrl: "data:img,b1" });
    s = applyAction(s, "submit_answer", "carol", { imageDataUrl: "data:img,c1" });
    // Alice cleared — still waiting on her before judging.
    assert.equal(s.phase, "answer");
  ...
  it("timer start and submit elapsed times", () => {
    let s = createInitialState(["alice", "bob"], { host: "alice" });
    const t0 = 1_000_000;
    s = applyAction(s, "start_timer", "bob", { now: t0 });
    assert.equal(s.timerStartedAt, t0);
    assert.throws(() => applyAction(s, "start_timer", "alice", { now: t0 + 100 }));
    s = applyAction(s, "submit_answer", "alice", {
      imageDataUrl: "data:a",
      now: t0 + 2000,
    });
    assert.equal(s.submissions.alice.elapsedMs, 2000);
    s = applyAction(s, "submit_answer", "alice", {
      imageDataUrl: "data:a2",
      now: t0 + 5000,
    });
    assert.equal(s.submissions.alice.elapsedMs, 5000);
    s = applyAction(s, "submit_answer", "bob", {
      imageDataUrl: "data:b",
      now: t0 + 7000,
    });
    assert.equal(s.phase, "judging");
    assert.equal(s.submissions.bob.elapsedMs, 7000);
    s = applyAction(s, "set_judgment", "alice", {
      targetUsername: "alice",
      value: "correct",
    });
    ...
    assert.equal(s.roundHistory[0].times.alice, 5000);
    assert.equal(s.timerStartedAt, null);
  });
});
